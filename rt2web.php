...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'RT' AND niveau = 2 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'RT' AND niveau = 2 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - RT2</title>
...
<body>
    ...
    <div id="showing">Currently showing: RT2</div>
    <?php
        $sansTP = ['Probabilités et Statistiques', 'Mathématiques du signal', 'Systèmes d\'exploitation', 'Architecture des ordinateurs', 'Techniques de transmission', 'Anglais', 'Gestion', 'Algèbre','Analyse mathématique','Electronique','Anglais','Français','Droit de l\'homme','Comptabilité','Onde et Propagation']; // Liste des matières sans TP
        $sansDS = ['Atelier Python Avancé', 'Atelier C++','Atelier Java','Developpement Web','Applications réparties','Système d\'exploitation UNIX']; 
        $sansEx = []; 
    ?>
