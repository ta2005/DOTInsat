...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'RT' AND niveau = 4 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'RT' AND niveau = 4 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - RT4</title>
...
<body>
    ...
    <div id="showing">Currently showing: RT4</div>
    <?php
        $sansTP = ['Théorie des langages et des automates','Anglais','Communication Interpersonnelle','Gestion financière','Droit des affaires','Gestion des ressources humaines','Management de projets','Management stratégique'];
        $sansDS = ['Projet Personnel Professionnel','Communication Interpersonnelle','Framework de développement','Management de projets','Fondements des systèmes et applications réparties']; 
