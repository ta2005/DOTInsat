...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IIA' AND niveau = 2 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IIA' AND niveau = 2 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - IIA2</title>
...
<body>
    ...
    <div id="showing">Currently showing: IIA2</div>
    <?php
        $sansTP = ['Probabilités & Statistiques', 'Traitement du signal', 'Réseaux informatiques','Mathématiques du signal', 'Systèmes d\'exploitation', 'Architecture des ordinateurs', 'Techniques de transmssion', 'Anglais', 'Gestion', 'Algèbre','Analyse Mathématiques','Anglais','Français','Droit de l\'homme','Comptabilité'];
        $sansDS = []; 
        $sansEx = []; 
    ?>
