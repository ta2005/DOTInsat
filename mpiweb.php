...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'MPI' AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'MPI' AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - MPI</title>
...
<body>
    ...
    <div id="showing">Currently showing: MPI</div>
    <?php
        $sansTP = ['Analyse 1','Algèbre 1','Optique géométrique','Mécanique générale','Algorithmique et Structures de données 1','Anglais','Français','Droit','Analyse 2','Algèbre 2','Algorithmique et Structure de données 2','Introduction à l\'économie'];
        $sansDS = ['Atelier de programmation 1','Atelier de programmation 2']; 
        $sansEx = []; 
    ?>
