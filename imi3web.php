...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 3 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 3 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - IMI3</title>
...
<body>
    ...
    <div id="showing">Currently showing: IMI3</div>
    <?php
        $sansTP = ['Mathématiques de l\'ingénieur','Anglais','Français','Arabe','Théorie des organisations','Communication dans l\'entreprise','Fiabilité des systèmes','Marketing','Tableau de bord de maintenance','Réseaux informatiques'];
        $sansDS = ['Marketing','Projet Personnel Professionnel']; 
