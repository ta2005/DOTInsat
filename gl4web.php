...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'GL' AND niveau = 4 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'GL' AND niveau = 4 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - GL4</title>
...
<body>
    ...
    <div id="showing">Currently showing: GL4</div>
    <?php
        $sansTP = ['Théorie des langages','Anglais','Communication Interpersonnelle','Gestion financière','Droit de l\'entreprise','Gestion des ressources humaines','Management de projets','Management stratégique'];
        $sansDS = ['Projet Personnel Professionnel','Développement d\'applications mobiles','Communication Interpersonnelle','Frameworks de développement II','Management de projets','Interfaces Homme-Machine (IHM)','Tests logiciels']; 
