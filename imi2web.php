...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 2 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 2 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - IMI2</title>
...
<body>
    ...
    <div id="showing">Currently showing: IMI2</div>
    <?php
        $sansTP = ['Probabilités et Statistiques', 'Mathématiques du signal', 'Systèmes d\'exploitation', 'Méthodes et moyens de maintenance', 'Electronique', 'Anglais', 'Gestion', 'Algèbre','Analyse Mathématiques','Anglais','Français','Droit de l\'Homme','Comptabilité','Traitement de signal','Propagation des ondes électromagnétiques']; 
        $sansDS = []; 
        $sansEx = []; 
    ?>
