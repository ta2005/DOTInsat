...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 4 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'IMI' AND niveau = 4 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - IMI4</title>
...
<body>
    ...
    <div id="showing">Currently showing: IMI4</div>
    <?php
        $sansTP = ['Sûreté de fonctionnement','Anglais','Communication Interpersonnelle','Gestion financière','Droit des affaires','Gestion des ressources humaines','Management de projets','Management stratégique','Mécanique des milieux continus (MMC)','Management environnemental','Outils et Moyens de contrôle de la qualité'];
        $sansDS = ['Projet Personnel Professionnel','Communication Interpersonnelle','Intelligence Artificielle','Systèmes temps réel','Capteurs intelligents et communicants']; 
