...
$reqS1 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'GL' AND niveau = 3 AND semestre = 1");
$reqS1->execute();
$matieresS1 = $reqS1->fetchAll(PDO::FETCH_ASSOC);

// Nrécupéri matiérét S2
$reqS2 = $bdd->prepare("SELECT nom_matiere, coefficient FROM matieres WHERE filiere = 'GL' AND niveau = 3 AND semestre = 2");
$reqS2->execute();
...
<head>
    <meta charset="utf-8">
    <title>INSAT Grade Calculator - GL3</title>
...
<body>
    ...
    <div id="showing">Currently showing: GL3</div>
    <?php
        $sansTP = ['Communication dans l\'entreprise','Anglais','Français','Théorie des organisations','Communication dans l\'entreprise','Arabe','Modélisation des systèmes','Mathématiques Statistiques','Logique','Complexité des algorithmes','Analyse numérique','Optimisation appliquée','Algorithmique avancée','Anglais','Marketing'];
        $sansDS = ['Projet Personnel Professionnel','Frameworks de développement', 'Unix: Administration système et réseau','Protocoles de communication Web-APIs', 'Bases de données non relationnelles']; 
        $sansEx = ['Projet Personnel Professionnel']; 
    ?>
